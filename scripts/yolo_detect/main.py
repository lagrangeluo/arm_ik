...
def train(args, params):
    ...
    # 加载预训练权重
    # pretrained_weights = '/home/guozz/Code/Python/SPGrid/weights/yolov8m-pose.pt'
    pretrained_weights = './weights/v8_m_pose.pt'
    checkpoint = torch.load(pretrained_weights, map_location='cuda')

    # 如果是完整模型对象，提取 state_dict
    if isinstance(checkpoint, dict) and "model" in checkpoint:
        if isinstance(checkpoint["model"], torch.nn.Module):
            state_dict = checkpoint["model"].state_dict()  # 从模型对象中提取权重
        else:
            state_dict = checkpoint["model"]  # 如果是 dict，直接使用
    elif isinstance(checkpoint, torch.nn.Module):
        state_dict = checkpoint.state_dict()  # 如果顶层是模型对象，提取权重
    else:
        state_dict = checkpoint  # 如果顶层是 state_dict，直接使用
    ...
    for name, param in model.named_parameters():
        if any(layer in name for layer in freeze_layers):  # 检查是否在需要冻结的模块中
    ...
    filenames = []
    with open('/home/guozz/Datasets/button_datasets/train.txt') as reader:
        for filename in reader.readlines():
            filename = filename.rstrip().split('/')[-1]
            filenames.append('/home/guozz/Datasets/button_datasets/RGB_Images/' + filename)
    ...
    with open('weights/step.csv', 'w') as f:
        ...
        for epoch in range(args.epochs):
            ...
            if epoch %10==0:
                torch.save(ckpt, './weights/{}.pt'.format(epoch))
            # Scheduler
            scheduler.step()
...
def demo(args):
    ...
    model = torch.load('./weights/v8_m_pose.pt', map_location='cuda')['model'].float()
