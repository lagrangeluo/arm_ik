...
class ImageProcessingServer:
    def __init__(self):
        ...
        rospy.loginfo("Color detector service up,waiting call...")
    ...
    def setup_result_directories(self):
        ...
        if not os.path.exists(base_dir):
            ...
            rospy.loginfo(f"Make dir: {base_dir}")
    ...
    def process_image(self, image, target_color):
        ...
        if target_color not in CONFIG['colors']:
            rospy.logerr(f"Unsupport color: {target_color}")
            return {}
    ...
    def handle_request(self, req):
        ...
        if self.image_subscriber:
            rospy.loginfo("Clean up subscriber....")
        ...
        rospy.loginfo(f"Received call，topic: {topic_name}，color：{color_type}")
        ...
        for attempt in range(max_attemp):
            ...
            if not self.received_image:
                rospy.logwarn(f"Try {attempt+1} times：timeout not received image")
                continue
            ...
            try:
                ...
                if result_json:
                    ...
                    if self.check_results_stability():
                        if self.image_subscriber:
                            rospy.loginfo("unregister subscriber")
                            self.image_subscriber.unregister()
                            self.image_subscriber = None
                            # 给ROS时间清理
                            rospy.sleep(0.05)
                        ...
                        rospy.loginfo(f"Detected result saved in: {saved_path}")
                    ...
                    if len(self.detection_results) > 0:
                        rospy.loginfo(f"Currnet frame can't detect,reseting...")
                        self.detection_results.clear()
                ...
                rospy.logerr(f"Try {attempt+1} image process error: {str(e)}")
                # 重置接收状态
                self.received_image = False
                self.current_image = None
                # rospy.sleep(0.2)
        ...
        rospy.logwarn("Max attemped,can't detect stable color")
        return None
